...
public boolean initCheck() {
    log.info("\n\t ################### InitCheck for [$navmap_1/$navmap_0]");
    if(pcfPool==null) return false;
    def conn = null;
    try {
        conn = pcfPool.borrowObject();
        def qmName = conn.getQManagerName();
        log.info("#### Init Check Passed: [$qmName] ####");
        log.info("#### PASS");
        return true;
    } catch (x) {
        log.info("#### FAIL");
        ...
        if(conn!=null) pcfPool.returnObject(conn);
    }
}

public boolean preExecCheck() {
    log.info("\n\t ****** PreExecCheck for [$navmap_1/$navmap_0]: [snafu : $snafu]");
    try {
        pcfPool.returnObject(pcfPool.borrowObject());
        return true;
    } catch (x) {
        return false;       
    }       
...
try {
   ...
   // TOPIC STATS
    ...
    //log.info "============= Done ============="    
    if(pcfConn!=null) try {pcfPool.returnObject(pcfConn);} catch (e) {}
}
